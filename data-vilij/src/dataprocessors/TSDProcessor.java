...
public final class TSDProcessor {
    ...
    public void toChartData(XYChart<Number, Number> chart) {
        ...
        for (String label : labels) {
            ...
            series.nodeProperty().get().setStyle("-fx-stroke: transparent;");
        }
    }
    ...
    public String getLabelNames(){
        ...
        for(Map.Entry<String, String> name: dataLabels.entrySet()){
            if(!list.contains(name.getValue())) {
                list.add(name.getValue());
                sw.append(" - " + name.getValue() + "\n");
            }
        }
        return sw.toString();
    }
    public void setData(Map<String, String> labels, Map<String, Point2D> points){
        this.dataLabels = labels;
        this.dataPoints = points;
    }
    public String[] getLabelList(){
        String[] list = new String[dataLabels.size()];
        int i = 0;
        for(Map.Entry<String, String> name: dataLabels.entrySet()){
            list[i] = name.getValue();
            i++;
        }
        return list;
    }
